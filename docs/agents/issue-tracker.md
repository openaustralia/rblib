...
on `openaustralia/rblib`. Use the `gh` CLI with an explicit `-R openaustralia/openaustralia` for every issue
operation; `gh`'s automatic repo inference from this clone would target the wrong repo.

When an issue is specific to this repo, name it (`rblib`) in the title or body so it's findable among the
...
Pull requests live on **this** repo (`openaustralia/rblib`) — only issues are centralised. When the flag is
